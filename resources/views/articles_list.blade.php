...
<head>
    ...
    <title>{{ $category->name }} — Artikel Investdu</title>
    <meta name="description" content="Baca artikel edukasi tentang {{ $category->name }} di Investdu. {{ $category->description }}">
...
<body>
    ...
    <section class="page-hero">
        ...
        <div class="hero-content">
            ...
            @if($category->badge)
                <div class="hero-badge">{{ $category->badge }}</div>
            @endif
            ...
            <div class="hero-stats">
                <div class="hero-stat">
                    ...
                    <div>
                        <div class="hero-stat-value">{{ $articles->count() }}</div>
                        <div class="hero-stat-label">Artikel</div>
                    </div>
                </div>
                <div class="hero-stat">
                    <div class="hero-stat-icon green">📖</div>
                    <div>
                        <div class="hero-stat-value">Gratis</div>
                        <div class="hero-stat-label">Akses penuh</div>
                    </div>
                </div>
            </div>

            <a href="{{ route('category.quiz', $category->slug) }}" class="btn-play-game">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                🎮 Mainkan Quiz
            </a>

            <form action="{{ route('articles.index', $category->slug) }}" method="GET" class="hero-search">
                <input type="text" name="q" value="{{ $query ?? '' }}" class="search-input" placeholder="Cari materi {{ $category->name }}..." autocomplete="off">
    ...
    <section class="articles-section">
        ...
        @if($articles->count() > 0)
            <div class="articles-grid">
                @foreach($articles as $article)
                    <a href="{{ route('articles.show', [$category->slug, $article->slug]) }}" class="article-card" id="article-{{ $article->id }}">
        ...
        @else
            <div class="empty-state">
                ...
                <p class="empty-desc">
                    @if(!empty($query))
                        Maaf, tidak ada artikel tentang {{ $category->name }} yang cocok dengan kata kunci <strong>"{{ $query }}"</strong>.
                        <br><br><a href="{{ route('articles.index', $category->slug) }}" style="color: #60A5FA; font-weight: 600; text-decoration: underline;">Tampilkan semua artikel</a>
                    @else
                        Artikel tentang {{ $category->name }} akan segera hadir. Nantikan update terbaru dari kami!
                    @endif
                </p>
            </div>
        @endif
